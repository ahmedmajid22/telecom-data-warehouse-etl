...
def build_customer_transactions(
    customers: pd.DataFrame, sim_cards: pd.DataFrame, transactions: pd.DataFrame
) -> pd.DataFrame:
    logging.info("Joining datasets to create analytics view")

    # Join sim_cards with customers
    customer_sim = sim_cards.merge(customers, on="customer_id", how="left")

    # Join transactions with the combined customer_sim data
    full_data = transactions.merge(customer_sim, on="sim_id", how="left")
